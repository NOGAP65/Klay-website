...
export default defineConfig({
  ...
  projects: [
    ...
    { name: 'forced-dark-android', testMatch: '**/visualiser-compatibility.spec.ts', use: { ...variants[1].settings, colorScheme: 'dark' } },
    ...variants.flatMap(({name, settings}) => (['light','dark'] as const).map(colorScheme => ({
      name: `${name}-${colorScheme}`, testMatch: ['**/site.spec.ts', '**/visualiser.spec.ts', '**/visualiser-compatibility.spec.ts', '**/shopping-experience.spec.ts'], use: { ...settings, colorScheme },
    }))),
  ],
});
