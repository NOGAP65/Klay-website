...
export const fieldsFor = (item: CatalogueItem): ConfigField[] => {
  ...
  // `opts` on this branch; the refactor branch renamed it `options`.
  if (opts.widths) {
    fields.push({
      id: 'width',
      label: 'Width',
  ...
  if (opts.hardwareChoices) {
    fields.push({
      id: 'hardware',
      label: opts.hardwareLabel ?? 'Hardware',
      kind: 'swatches',
      choices: opts.hardwareChoices,
    })
