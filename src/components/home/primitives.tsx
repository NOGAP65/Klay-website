...
export function PhotoTile({
   ...
   * and what it promises is "show me this one". A tile that navigates with an
   * expander underneath it would be two controls disagreeing about what the
   * card is for. */
  onActivate?: () => void;
  label: string;
  ...
  // A <button> where there is no destination, an <a> where there is. Widened to
  // ElementType because Link and 'button' take genuinely different props and
  // TypeScript cannot narrow a union of components by the value of a variable;
  // the pairing is right by construction on the next line.
  ...
  return (
    <Tag
      ...
      {...nav}
      style={{
        // Reset, so a button lays out identically to the link — the styles
        // below are the tile.
        ...(onActivate
          ? {
              padding: 0, border: 'none', background: 'none', font: 'inherit',
              color: 'inherit', textAlign: 'left' as const, width: '100%',
              cursor: 'pointer',
            }
          : null),
        position: 'relative',
