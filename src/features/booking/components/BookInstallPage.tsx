...
export default function BookInstallPage() {
  ...
  async function submit(mode: Mode) {
    if (busy) return;
    setFormError(null);
    if (isBasketQuote && (!isQuoteItems(items) || mode === 'pay')) {
      setFormError('Please check your basket before requesting a quote.');
      return;
    }
      ...
      <main style={{ background: tokens.paper, paddingTop: 140, paddingBottom: 120 }}>
        <div style={{ maxWidth: layout.containerMax, margin: '0 auto', padding: '0 5vw' }}>
          <div style={{ maxWidth: 640, marginBottom: 56 }}>
            ...
            <h1 style={{ ...headline.section, color: tokens.ink, marginBottom: 20 }}>
              {isBasketQuote ? 'Your products. One measure.' : 'Two ways to go ahead.'}
            </h1>
            <p style={{ fontFamily: tokens.body, fontSize: 15, lineHeight: 1.75, color: tokens.inkSoft }}>
              {isBasketQuote ? 'Send your selected products and finishes to our team. We’ll confirm the details and pricing after measuring.' : <>Ask us for a quote and we'll confirm the price after measuring, or pay now and
            ...
            >
              ...
              {isBasketQuote ? <>
                {items.length ? items.map((item, index) => <div key={index} style={{ color: tokens.ink, marginBottom: 20 }}>
                  <h3 style={{ fontFamily: tokens.body, fontSize: 15 }}>{item.name} × {item.quantity}</h3>
              ...
              <button
                ...
                disabled={busy !== null || (isBasketQuote && !items.length)}
                style={{
                  width: '100%',
                  marginTop: 12,
                  padding: '16px',
                  background: isBasketQuote ? tokens.accent : 'transparent',
                  color: isBasketQuote ? tokens.onAccent : tokens.ink,
                  fontFamily: tokens.body,
                  fontSize: 11,
                  fontWeight: 600,
                  letterSpacing: '0.14em',
              ...
              >
                {busy === 'quote' ? 'Sending…' : isBasketQuote ? 'Request a quote' : 'Request a quote instead'}
              </button>
