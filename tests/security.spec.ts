...
test('a verified enquiry stores only approved columns and escapes customer text in both emails', async () => {
  ...
  // Neither email may ever carry live markup.
  for (const mail of mails) {
    expect(mail.html).not.toContain('<script>');
    expect(mail.html).not.toContain('<img src=x');
  }
  // The internal alert carries the customer's text, escaped. The customer
  // acknowledgement does not carry it at all — see customerSummary() in
  // netlify/lib/notify.ts for why echoing to an unverified address is the
  // relay, and the phishing-relay test below for the measured payload.
  const toKlay = mails.find(mail => !mail.to.includes(valid.email))!;
  const toCustomer = mails.find(mail => mail.to.includes(valid.email))!;
  expect(toKlay.html).toContain('&lt;script&gt;');
  expect(toCustomer.html).not.toContain('&lt;script&gt;');
  expect(toCustomer.html).not.toContain('alert(1)');
});
