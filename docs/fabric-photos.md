...
The exterior window photograph and its material mask are now managed by
`shopPhotos.ts` and `ShopPhotoLayers.tsx`. The former interior photograph and
its raster mask have been removed. See [shop-photos.md](shop-photos.md) for the
new assets, colour treatment and verification commands.
