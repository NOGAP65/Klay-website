...
Manual outdoor previews use an illustrated crank on the left edge of the photo. Motorised selections replace it with the existing remote, in a compact case for narrow previews. No operation choices were added. Every new control accepts a simple straight drag: up opens, down closes. The illustrated hardware moves in response; customers do not need to trace circles or learn different gestures. Larger Open/Close buttons finish the movement with one tap in 350ms, or immediately when reduced motion is enabled. Grabbing the control, pressing a key or switching products cancels that movement. Venetians retain separate lift and tilt controls; plantations have a hand-operated louvre illustration. Roller blind and curtain controls are unchanged. Vector artwork adds no image downloads; animation only runs during a requested movement.
