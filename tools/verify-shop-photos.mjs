...
for (const photo of photos) {
  ...
  assert.ok(photo.regions?.length || photo.boards?.length || photo.shower || photo.semi || photo.mirror, `${photo.src} needs material regions`);
}
...
assert.equal(new Set(photos.map(p => p.src)).size, 20, 'Both mirrors share one photo');
console.log('Shop photos: 20 product photos for 21 configurations and four supporting assets; layouts, exclusions, colours and shadows pass.');
