...
// WHAT THE CLOTH ACTUALLY PHOTOGRAPHS AS, per shot — the white point the card
// has to divide out before it multiplies.
//
// THE HEADER OF fabricShots SAYS THE FABRIC "HAS BEEN NORMALISED TO WHITE". It
// has not, and that is the bug these numbers fix. Measured through each
// product's own mask, taking only pixels the mask marks solidly inside so the
// feathered edge cannot drag the average onto the wall:
//
//   roller blockout     223,217,211      curtains blockout   203,194,182
//   roller lightfilter  240,235,228      curtains sheer      230,227,222
//   roller sunscreen    216,208,195      venetian            221,215,203
//   roller dual         200,191,176      plantation          208,201,189
//   awning              223,214,202
//
// The medians of the same samples are 222 for the roller, 200 for the curtains,
// 197 for the shutters and 227 for the venetian — the figures already written
// into the SHEEN note above by hand, which is what says this measurement agrees
// with the one that was done before.
//
// EVERY ONE OF THEM IS WARM. Red runs ahead of blue by 12 on the roller
// blockout and by 24 on the dual, and a multiply through that lands the cast on
// whatever the customer picked. It shows up worst on the pale end, where there
// is nothing else in the colour to hide it: White measured out of the rendered
// card at 222,216,207 against a swatch of 242,240,236 — an error of -20,-24,-29,
// which is not "slightly dark", it is cream. Every colour was carrying it in
// proportion; see dyePaint in ShopCard for what is done about it.
//
// PER SHOT, NOT PER PRODUCT, unlike everything above. The four roller fabrics
// are four different cloths photographed in one window and they run from 240
// down to 200 — the same correction cannot serve a light filter and a dual.
const WHITE_POINT = {
  'roller-blinds-blockout.webp': [223, 217, 211],
  'roller-blinds-lightfilter.webp': [240, 235, 228],
  'roller-blinds-sunscreen.webp': [216, 208, 195],
  'roller-blinds-dual.webp': [200, 191, 176],
  'curtains-blockout.webp': [203, 194, 182],
  'curtains-sheer.webp': [230, 227, 222],
  'venetian-blinds.webp': [221, 215, 203],
  'plantation-shutters.webp': [208, 201, 189],
  'folding-arm-awnings.webp': [223, 214, 202],
};
...
const body = shots.map(s =>
  ...
  `spec: ${SPECULAR[s.product] ?? 0.4}` +
  // Only a dyed shot has one, and an undyed shot has nothing to divide out.
  // Absent means the card paints the swatch straight through, which is what
  // every card did before this existed.
  `${WHITE_POINT[s.file] ? `, white: [${WHITE_POINT[s.file].join(', ')}]` : ''} },`).join('\n');
