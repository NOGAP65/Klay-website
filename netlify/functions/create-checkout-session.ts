...
export default async (req: Request, context?: Pick<Context, 'ip'>): Promise<Response> => {
  ...
  const rateLimited = checkRateLimit(req, context?.ip)
  if (rateLimited) return rateLimited

  const body = await readJson(req)
  if (body instanceof Response) return body
  if (body.items !== undefined) return badRequest('Basket configurations must be submitted as a quote request.')
  if (['blindType', 'windowSize', 'operation', 'quantity'].some(key => body[key] === undefined)) {
    return badRequest('Please choose the product, size, operation and quantity before checkout.')
  }
  ...
  const turnstileError = await verifyTurnstile(body, getClientIp(req, context?.ip), req)
  ...
  try {
    ...
    if (site.username || site.password || (site.protocol !== 'https:' && !isLocal)) throw new Error('Invalid site URL')
    siteOrigin = site.origin
  } catch { return notConfigured(['SITE_URL']) }
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${siteOrigin}/booking/confirmed?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${siteOrigin}/book?${cancelParams}`,
    ...
    if (linkError) console.error('[checkout] could not attach session id')
